...
import { fetchPersonnelRoster, submitRoleChange } from './actions'
...
export function PersonnelRosterPanel() {
  const [users, setUsers] = useState<User[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    fetchPersonnelRoster().then(data => {
          ...
          <tbody className="text-sm">
            {users.map(u => (
              <tr key={u.user_id} className="border-b border-border/50 hover:bg-surface-hover/50 transition-colors">
                ...
                <td className="py-4">
                  <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${u.status === 'active' ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}`}>
                    {u.status}
                  </span>
                </td>
                <td className="py-4">
                  <div className="flex flex-wrap gap-2">
                    ...
                    <div className="relative group inline-block">
                      <span className="inline-flex items-center justify-center rounded border border-dashed border-border px-2 py-1 text-xs cursor-pointer hover:bg-surface-hover hover:border-accent text-muted transition-all">
                        + Add
                      </span>
                      <div className="absolute left-0 top-full mt-1 hidden group-hover:flex flex-col bg-surface border border-border rounded-lg shadow-xl z-50 min-w-[120px] overflow-hidden">
